    ...
    public static void createTable(SQLiteDatabase db) {
        try {
            db.execSQL("create table " + TABLE_NAME
                    + "(_id integer primary key autoincrement, id integer, tag text, uri text, start long, end long, finished long ,onlywifi integer)");
            ...
            try {
                db.execSQL(
                        "insert into " + TABLE_NAME
                                + "(id, tag, uri, start, end, finished ,onlywifi) values(?, ?, ?, ?, ?, ? ,?)",
                        new Object[] {info.getId(), info.getTag(), info.getUri(), info.getStart(), info.getEnd(),
