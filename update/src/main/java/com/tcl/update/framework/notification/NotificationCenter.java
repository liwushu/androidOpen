package com.tcl.update.framework.notification;
...
public class NotificationCenter {
    ...
    public void setLooper(Looper looper) {
        ...
        if (mHandler.getLooper() == looper) return;
    ...
    protected boolean subscribe(final Object classTopicOrPatternWrapper, final Map<Object, Object> subscriberMap,
        ...
        synchronized (listenerLock) {
            ...
            if (currentSubscribers == null) {
                NLog.d(TAG, "Creating new subscriber map for:" + classTopicOrPatternWrapper);
    ...
    public boolean subscriber(Class eventClass, Subscriber subscriber) {
        ...
        if (NLog.isDebug()) {
            NLog.d(TAG, "Subscribing by class, class:" + eventClass + ", subscriber:" + subscriber);
        }

        return subscribe(eventClass, subscribersByClass, new WeakReference<Subscriber>(subscriber));
    }
    ...
    public boolean subscriber(String topic, TopicSubscriber subscriber) {
        ...
        if (NLog.isDebug()) {
            NLog.d(TAG, "Subscribing by topic, topic:" + topic + ", subscriber:" + subscriber);
        }

        return subscribe(topic, subscribersByTopic, new WeakReference<TopicSubscriber>(subscriber));
    }

    /**
     * All event subscriber unsubscriptions call this method. Extending classes only have to
     * override this method to subscribe all subscriber unsubscriptions.
     * 
    ...
    protected boolean unsubscribe(Object o, Map subscriberMap, Object subscriber) {
        if (NLog.isDebug()) {
            NLog.v(TAG, "unsubscribe(" + o + "," + subscriber + ")");
        }
    ...
    /**
     * All publish methods call this method. Extending classes only have to override this method to
     * handle all publishing cases.
     * 
     ...
     */
    @SuppressWarnings("unchecked")
    protected void publish(final Object event, final String topic, final Object eventObj, final List subscribers) {
        ...
        if (subscribers == null || subscribers.isEmpty()) {
            if (NLog.isDebug()) {
                NLog.d(TAG, "No subscribers for event or topic. Event:" + event + ", Topic:" + topic);
            }
            return;
        }
    ...
    class PublishRunnable implements Runnable {
        ...
        public PublishRunnable(final Object event, final String topic, final Object eventObj, final List subscribers) {
            this.theEvent = event;
            this.theTopic = topic;
