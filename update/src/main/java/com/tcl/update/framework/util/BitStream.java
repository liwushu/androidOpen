 ...
 * 
 * History: 1.0 devilxie 2012-09-29 Create
 */
...
public class BitStream {
    ...
    public boolean endWiths(byte[] b) {
        if (nBufferedBitsLen < b.length * 8) return false;

        if (iBitsWriteOffset == 0) {
            for (int i = 1; i < (b.length + 1); i++) {
                if (bitsBuffer[nCurrentWriteByte - i] != b[b.length - i]) return false;
            }
        }
    ...
    public int read(byte[] buffer, int start) {
        ...
        if (start < 0 || start >= buffer.length) throw new ArrayIndexOutOfBoundsException(start);
    ...
    public long readBits(int bits) {
        long ret = 0;
        if (bits <= 0) throw new IllegalArgumentException("bits must > 0");
    ...
    public byte[] toByteArray() {
        if (nBufferedBitsLen == 0) return new byte[0];
     ...
     */
    public void compact() {
        if (nCurrentReadByte == 0 || nCurrentWriteByte == 0) return;
