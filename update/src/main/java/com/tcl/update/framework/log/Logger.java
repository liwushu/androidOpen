 ...
 */
package com.tcl.update.framework.log;
...
public class Logger {
    /**
     * 日志跟踪级别 TRACE_REALTIME 表示实时跟踪，将只显示在logcat中 TRACE_OFFLINE 表示离线跟踪，将只保存在特定日志文件中 TRACE_ALL 表示都跟踪
     */
    ...
    /**
     * 日志优先级， 用于println，等同于Log.d，属于调试的内容，一般用于调试跟踪日志。主要用于开发，产品发布阶段建议把该 部分日志删除
     */
    ...
    public int v(String tag, String msg) {
        ...
        int ret = 0;
        if ((trace & TRACE_REALTIME) > 0) ret = Log.v(tag, msg);

        if ((trace & TRACE_OFFLINE) > 0) ret = println(VERBOSE, tag, msg);
    ...
    public int d(String tag, String msg) {
        ...
        int ret = 0;
        if ((trace & TRACE_REALTIME) > 0) ret = Log.d(tag, msg);

        if ((trace & TRACE_OFFLINE) > 0) ret = println(DEBUG, tag, msg);
    ...
    public int w(String tag, String msg) {
        if (WARN < level) return 0;
        int ret = 0;
        if ((trace & TRACE_REALTIME) > 0) ret = Log.w(tag, msg);

        if ((trace & TRACE_OFFLINE) > 0) ret = println(WARN, tag, msg);
    ...
    public int i(String tag, String msg) {
        ...
        int ret = 0;
        if ((trace & TRACE_REALTIME) > 0) ret = Log.i(tag, msg);

        if ((trace & TRACE_OFFLINE) > 0) ret = println(INFO, tag, msg);
    ...
    public int e(String tag, String msg) {
        ...
        if (ERROR < level) return 0;
        int ret = 0;
        if ((trace & TRACE_REALTIME) > 0) ret = Log.e(tag, msg);

        if ((trace & TRACE_OFFLINE) > 0) ret = println(ERROR, tag, msg);
