 ...
 * 
 * FileName:IPAddressUtils.java
 * 
 * Description:IP地址字符串和数值形式之间转换。
 * 
 * History: 1.0 Nongfuliu 2012-09-06 Create
 */

package com.tcl.update.framework.network;
...
public class IPAddressUtils {
     ...
     * 把字符串类型的IP地址转换成数值。
     * 
     * @param ipStr 要转换的IP地址字符串
     * @return IP地址的数值表示
     */
    public static long ipToLong(String ipStr) {
        if (TextUtils.isEmpty(ipStr)) throw new IllegalArgumentException("empty ip!");

        int[] ip = new int[4];
        String[] ips = ipStr.split(".");

        if (ips == null || ips.length != 4) {
            NLog.w(TAG, "wrong ip string format!");
            return 0;
        }
